...
class Cnn(object):
    def __init__(self, sequence_length, vocab_size, embedding_size,
        ...
        num_filters_total = num_filters * len(filter_sizes) # 10 * ４
        ...
        with tf.name_scope("output"):
            Wa = tf.Variable(tf.truncated_normal([None, num_filters_total], stddev=0.1), name="Wa")
            Wb = tf.Variable(tf.truncated_normal([None, num_filters_total], stddev=0.1), name="Wa")
            ba = tf.Variable(tf.constant(0.1, shape=[None]), name="ba")
            bb = tf.Variable(tf.constant(0.1, shape=[None]), name="bb")
            self.mul_a = tf.add(tf.multiply(Wa, self.h_pool_flat_a), ba)
            self.mul_b = tf.add(tf.multiply(Wb, self.h_pool_flat_b), bb)
            self.result = self.add(self.mul_a, self.mul_b)
